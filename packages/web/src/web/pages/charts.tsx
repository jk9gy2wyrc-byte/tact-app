...
function downsample(arr: number[], maxPts: number): { idx: number; val: number }[] {
  ...
  for (let i = step - 1; i < arr.length; i += step) result.push({ idx: i + 1, val: arr[i] });
  return result;
}
...
function DeviationSummary({ btSeries, lvSeries, mcpSeries, unit = '' }: {
  ...
  const inBand = lastP5 != null && lastP95 != null
    ? (lastLv >= lastP5 && lastLv <= lastP95 ? 'У межах норми' : lastLv < lastP5 ? 'Нижче p5 — увага!' : 'Вище p95 — увага!') : null;
        ...
        <div style={{ marginTop: 8, padding: '12px 16px', background: 'var(--surface2)', borderRadius: 10, border: '1px solid var(--border)', fontSize: 12, lineHeight: 1.8 }}>
          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
            ...
            <div>
              ...
              <div className="mono" style={{ color: devBT != null ? color(devBT) : 'var(--text2)' }}>{devBT != null ? fmtPct(devBT) : '—'}</div>
            </div>
            {devMed != null && (
              <div>
...
function MetricChart({ title, btSeries, lvSeries, mcpSeries, refY, unit = '', height = 220, explanation, isMobile = false }: {
  ...
  const btDs    = downsample(btSeries, 120);
  const lvDs    = downsample(lvSeries, 60);
  const mcMedDs = mcpSeries ? downsample(mcpSeries.med, 60) : [];
  const mcp5Ds  = mcpSeries ? downsample(mcpSeries.p5,  60) : [];
  const mcp95Ds = mcpSeries ? downsample(mcpSeries.p95, 60) : [];
...
const defaultStress = {
  ...
  lossClustering: 0, clusterSize: 3, regimeShiftWR: 0,
...
export default function Charts() {
  ...
  const mapMCtoRolling = (btLen: number, mc: { med: number[]; p5: number[]; p95: number[] }) => {
    ...
    <div style={{ padding: isMobile ? '12px 10px' : '24px 28px', overflowX: 'hidden', boxSizing: 'border-box', width: '100%' }}>
          ...
          <>
            <ResponsiveContainer width="100%" height={isMobile ? 220 : 340}>
              <LineChart data={eqData} margin={{ top: 4, right: 16, bottom: 4, left: 8 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#2a2d33" />
                <XAxis dataKey="trade" stroke="#5a5f6a" tick={{ fontSize: 10, fill: '#8b9098' }}
                  tickFormatter={equityViewMode === 'normalized' ? (v: number) => `${v}%` : undefined} />
                <YAxis stroke="#5a5f6a" tick={{ fontSize: 10, fill: '#8b9098' }}
                  tickFormatter={equityViewMode === 'normalized' ? (v: number) => `${v.toFixed(2)}R` : undefined} />
                <Tooltip content={<DeviationTooltip />} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 20 }}>
        <MetricChart
          ...
          mcpSeries={wrMC ? { med: wrMC.med.map(v => Math.round(v * 1000) / 10), p5: wrMC.p5.map(v => Math.round(v * 1000) / 10), p95: wrMC.p95.map(v => Math.round(v * 1000) / 10) } : undefined}
          refY={50} unit="%" isMobile={isMobile}
        ...
        <MetricChart
          ...
          btSeries={btRolling.avgRR} lvSeries={lvRolling.avgRR} mcpSeries={rrMC} refY={1} isMobile={isMobile}
        ...
        <MetricChart
          ...
          btSeries={btRolling.pf} lvSeries={lvRolling.pf} mcpSeries={pfMC} refY={1} isMobile={isMobile}
        ...
        <MetricChart
          ...
          btSeries={btRolling.maxDD} lvSeries={lvRolling.maxDD} mcpSeries={ddMC} refY={0} isMobile={isMobile}
      ...
      <MetricChart
        ...
        btSeries={btRolling.stdDev} lvSeries={lvRolling.stdDev} mcpSeries={sdMC} refY={0}
        height={isMobile ? 160 : 200} isMobile={isMobile}
      ...
      <div style={chartStyle(isMobile)}>
        ...
        <div style={{ fontSize: 11, color: 'var(--text2)', marginBottom: 12 }}>
          1000 симуляцій · тьмяні лінії = 100 окремих шляхів · білий = медіана · помаранчевий = p5/p95
        </div>
        {mcMed.length === 0 ? (
      ...
      <div style={{ ...chartStyle(isMobile), border: stressOpen ? '1px solid #f8717155' : '1px solid var(--border)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer', userSelect: 'none' }} onClick={() => setStressOpen(o => !o)}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {isModified && <span style={{ background: '#f8717122', color: '#f87171', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, border: '1px solid #f8717144' }}>ACTIVE</span>}
          </div>
