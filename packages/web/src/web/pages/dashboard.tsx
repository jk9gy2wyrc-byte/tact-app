...
function TopGainerCard({ trades, label }: { trades: any[]; label: string }) {
  ...
  if (!trades.length) return (
    <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ color: 'var(--text2)', fontSize: 12 }}>No data yet.</div>
    </div>
  );

  // aggregate netR per asset
  const byInst: Record<string, { netR: number; trades: number[] }> = {};
  for (const t of trades) {
    const k = ((t.asset as string | null) ?? '—').toUpperCase();
    if (!byInst[k]) byInst[k] = { netR: 0, trades: [] };
    byInst[k].netR += t.netR ?? 0;
    byInst[k].trades.push(t.netR ?? 0);
  }

  const rows = Object.entries(byInst)
    .map(([k, v]) => ({ key: k, netR: Math.round(v.netR * 100) / 100, trades: v.trades }))
    .sort((a, b) => b.netR - a.netR);

  const winner = rows[0];
  if (!winner) return null;

  // build cumulative equity curve for the winner
  const curve: number[] = [];
  let cum = 0;
