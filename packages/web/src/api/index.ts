...
const app = new Hono()
  ...
  .get('/auth/access/:userId', async (c) => {
    ...
    if (user.role === 'admin') {
      return c.json({ hasAccess: true, reason: 'admin' }, 200);
    }
    
    // Check if suspended
    if (user.accessStatus === 'suspended') {
      return c.json({ hasAccess: false, reason: 'suspended' }, 200);
    }
    ...
    if (user.accessExpiryDate) {
      ...
      const now = new Date();
      if (now > expiryDate) {
        return c.json({ hasAccess: false, reason: 'expired' }, 200);
      }
    }
    
    return c.json({ hasAccess: true, reason: 'active' }, 200);
  })
  ...
  .post('/auth/register',
    ...
    async (c) => {
      ...
      // Set 1 week free access from now
      ...
      const [newUser] = await db.insert(users).values({
        ...
        password,
        role: 'user',
        accessExpiryDate: oneWeekFromNow.toISOString(),
        accessStatus: 'active',
      }).returning();
  ...
  // ─── ADMIN: update user notes and access ───────────────────────────────────────
  .put('/admin/users/:id',
    ...
    async (c) => {
      ...
      const updateData: { notes?: string; accessExpiryDate?: string; accessStatus?: string } = {};
      if (body.notes !== undefined) updateData.notes = body.notes;
      if (body.accessExpiryDate !== undefined) updateData.accessExpiryDate = body.accessExpiryDate;
      if (body.accessStatus !== undefined) updateData.accessStatus = body.accessStatus;
      const [updated] = await db.update(users).set(updateData).where(eq(users.id, id)).returning();
