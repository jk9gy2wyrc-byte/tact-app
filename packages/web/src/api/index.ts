...
const app = new Hono()
  ...
  .get('/auth/seed', async (c) => {
    ...
    const existing = await db.select().from(users).where(eq(users.login, 'vygnanskyi@gmail.com')).get();
    if (!existing) {
      await db.insert(users).values({ login: 'vygnanskyi@gmail.com', password: '7777', role: 'admin', email: 'vygnanskyi@gmail.com' });
    } else if (existing.role !== 'admin') {
      await db.update(users).set({ role: 'admin' }).where(eq(users.login, 'vygnanskyi@gmail.com'));
    }
